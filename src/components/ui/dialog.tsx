	...
	<DialogPortal>
		...
		>
			<div className="relative rounded-lg border border-border bg-background shadow-lg">
				{showCloseButton && (
					<DialogPrimitive.Close className="absolute right-6 top-6 rounded-full hover:bg-secondary p-2 transition-colors">
...
const DialogHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
	<div className={cn("flex flex-col gap-3 mb-6", className)} {...props} />
);
...
>(({ className, ...props }, ref) => (
	<DialogPrimitive.Description ref={ref} className={cn("text-sm text-primary", className)} {...props} />
));
